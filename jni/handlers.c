...
extern struct result * resultbuf;
...
bool handler__set(globals_t * vars, char **argv, unsigned argc) {
	...
	if ((vars->options.scan_data_type == BYTEARRAY)
		...
		show_error(
				"`set` is not supported for bytearray or string, use `write` instead.\n");
		return false;
	}
	...
	for (block = 0; block < argc - 1; block++) {
		...
		if ((settings[block].value = strchr(argv[block + 1], '=')) == NULL) {
			...
			settings[block].matchids = strndupa(argv[block + 1],
		...
		if ((delay = strchr(settings[block].value, '/')) != NULL) {
			...
			if (*(delay + 1) == '\0') {
				...
				show_error(
						"you specified an empty delay count, `%s`, see `help set`.\n",
			...
			settings[block].value = strndupa(settings[block].value,
			...
			if (settings[block].matchids != NULL) {
				...
				while ((id = strtok(lmatches, ",")) != NULL) {
					...
					if (loc.swath) {
						value_t v;
						value_t old;
						void *address = remote_address_of_nth_element(loc.swath,
								loc.index /* ,MATCHES_AND_VALUES */);

						/* copy val onto v */
						/* XXX: valcmp? make sure the sizes match */
						old = data_to_val(loc.swath,
								loc.index /* ,MATCHES_AND_VALUES */);
						v.flags = old.flags =
								loc.swath->data[loc.index].match_info;
				...
				while (reading_swath_index->first_byte_in_child) {
					...
					if (flags_to_max_width_in_bytes(
							...
							> 0) {
						void *address = remote_address_of_nth_element(
								reading_swath_index,
					...
					++reading_iterator;
					if (reading_iterator
							>= reading_swath_index->number_of_bytes) {
		...
		if (flags_to_max_width_in_bytes(flags) > 0) {
			...
			case BYTEARRAY:
				...
				data_to_bytearray_text(v, buf_len, reading_swath_index,
						reading_iterator, flags.bytearray_length);
				assert(strlen(v) + strlen(bytearray_suffix) + 1 <= buf_len);
				/* or maybe realloc is better? */
			...
			default: /* numbers */
				; /* cheat gcc */
				value_t val = data_to_val(reading_swath_index,
						reading_iterator /* ,MATCHES_AND_VALUES */);
...
#define POINTER_FMT "%20p"
			...
			fprintf(stdout, "[%2u] "POINTER_FMT", %s\n", i++,
					remote_address_of_nth_element(reading_swath_index,
							reading_iterator /* ,MATCHES_AND_VALUES */), v);
		}
		...
		if (reading_iterator >= reading_swath_index->number_of_bytes) {
			assert(
					((matches_and_old_values_swath *)(local_address_beyond_last_element(reading_swath_index /* ,MATCHES_AND_VALUES */)))->number_of_bytes >= 0);
			reading_swath_index = local_address_beyond_last_element(
					reading_swath_index /* ,MATCHES_AND_VALUES */);
			reading_iterator = 0;
		}
...
/* XXX: handle multiple deletes, eg delete !1 2 3 4 5 6 */bool handler__delete(
		globals_t * vars, char **argv, unsigned argc) {
	unsigned id;
	char *end = NULL;
	match_location loc;
	...
	if (loc.swath) {
		...
		memset(&(loc.swath->data[loc.index].match_info), 0,
				sizeof(match_flags));
		return true;
	} else {
		...
		show_info(
				"use \"list\" to list matches, or \"help\" for other commands.\n");
		return false;
	}
}
...
bool handler__reset(globals_t * vars, char **argv, unsigned argc) {
	...
	if (vars->target && readmaps(vars->target, vars->regions) != true) {
		show_error(
				"sorry, there was a problem getting a list of regions to search.\n");
...
/* dregion [!][x][,x,...] */bool handler__dregion(globals_t * vars, char **argv,
		unsigned argc) {
	unsigned id;
	bool invert = false;
		...
		if (*end != '\0' || *idstr == '\0') {
			...
			if (invert) {
				if (l_concat(vars->regions, &keep) == -1) {
					show_error(
							"there was a problem restoring saved regions.\n");
					l_destroy(vars->regions);
					l_destroy(keep);
					return false;
				}
...
bool handler__lregions(globals_t * vars, char **argv, unsigned argc) {
	...
	while (np) {
		...
		fprintf(stderr, "[%2u] %#10lx, %7lu bytes, %c%c%c, %s\n", region->id,
				...
				region->flags.read ? 'r' : '-', region->flags.write ? 'w' : '-',
				region->flags.exec ? 'x' : '-',
...
/* the name of the function is for history reason, now GREATERTHAN & LESSTHAN are also handled by this function */bool handler__decinc(
		globals_t * vars, char **argv, unsigned argc) {
	uservalue_t val;
	scan_match_type_t m;
...
bool handler__help(globals_t * vars, char **argv, unsigned argc) {
	...
	FILE *outfd = popen("busybox more", "w");
	if (outfd == NULL) {
	...
	while (np) {
		...
		if (argv[1] == NULL) {
			...
			fprintf(outfd, "%s\n",
					command->longdoc ?
							command->longdoc : "missing documentation");
			ret = true;
			goto retl;
		}
...
/* XXX: handle !ls style escapes */bool handler__shell(globals_t * vars,
...
bool handler__watch(globals_t * vars, char **argv, unsigned argc) {
	...
	address = remote_address_of_nth_element(loc.swath,
			loc.index /* ,MATCHES_AND_VALUES */);
	...
	while (true) {
		...
		scan_routine_t valuecmp_routine = (get_scanroutine(ANYNUMBER,
				MATCHCHANGED));
...
bool handler__write(globals_t * vars, char **argv, unsigned argc) {
	...
	if ((strcasecmp(argv[1], "i8") == 0)
		...
		for (i = 0; i < 3; ++i) {
			while (((*string_parameter) == ' ') || (*string_parameter) == '\t')
				++string_parameter;
			while (((*string_parameter) != ' ') && (*string_parameter) != '\t')
				++string_parameter;
		}
	...
	if(ret)
	{
		sprintf(resultbuf->buffer,"%s","SUCCESS");
		resultbuf->size = 7;
	}else{
		sprintf(resultbuf->buffer,"%s","FAILED");
		resultbuf->size = 5;
	}
...
bool handler__option(globals_t * vars, char **argv, unsigned argc) {
	...
	if (strcasecmp(argv[1], "scan_data_type") == 0) {
		...
		if (strcmp(argv[2], "0") == 0) {
			...
			show_error(
					"bad value for detect_reverse_change, see `help option`.\n");
			return false;
		}
	} else {
 ...
 */bool handler__read(globals_t * vars, char **argv, unsigned argc) {
	...
	read_array(vars->target, addr, buf, data_width);

	len = data_width;

	destbuf = (char*) malloc((len) * sizeof(char));
	...
	for (i = 0; i < data_width; i++) {
		sprintf(d, "%02X", (unsigned char) (destbuf[i]));
		d += 2;
	}
	printf("%s\n", buf);

	if ((strcasecmp(argv[1], "i32") == 0)
			|| (strcasecmp(argv[1], "int32") == 0)) {
		sscanf(buf, "%x", &nValue);
		sprintf(resultbuf->buffer, "%d\n", *((int*) &nValue));
		printf("int %s %d\n", resultbuf->buffer, *((int*) &nValue));
	} else if ((strcasecmp(argv[1], "f32") == 0)
			|| (strcasecmp(argv[1], "float32") == 0)) {
		sscanf(buf, "%x", &fValue);
		sprintf(resultbuf->buffer, "%f\n", *((float*) &fValue));
		printf("float %s %f\n", resultbuf->buffer, *((float*) &fValue));
	} else if ((strcasecmp(argv[1], "f64") == 0)
			|| (strcasecmp(argv[1], "float64") == 0)) {
		sscanf(buf, "%x", &dValue);
		sprintf(resultbuf->buffer, "%lf\n", *((double*) &dValue));
		printf("double %s %lf\n", resultbuf->buffer, *((double*) &fValue));
	} else if ((strcasecmp(argv[1], "i64") == 0)
			|| (strcasecmp(argv[1], "int64") == 0)) {
		sscanf(buf, "%x", &lValue);
		sprintf(resultbuf->buffer, "%ld\n", *((long*) &dValue));
		printf("double %s %ld\n", resultbuf->buffer, *((long*) &fValue));
	}

	resultbuf->size = data_width;
	retl: if (buf)
		free(buf);
	if (destbuf)
		free(destbuf);
