...
bool peekdata(pid_t pid, void *addr, value_t * result) {
	...
	if (pid == peekbuf.pid && reqaddr >= peekbuf.base
			&& (unsigned long) (reqaddr + sizeof(int64_t) - peekbuf.base)
					<= peekbuf.size) {

		result->int64_value = *((int64_t *) &peekbuf.cache[reqaddr
				- peekbuf.base]); /*lint !e826 */
		return true;
		...
		shift_size1 = (reqaddr + sizeof(int64_t))
				- (peekbuf.base + peekbuf.size);
	...
	/* we need a ptrace() to complete request */errno = 0;
		...
		if (EXPECT(ptraced_long == -1L && errno != 0, false)) {
			...
			if (errno == EIO || errno == EFAULT) {
				...
				for (j = 1, errno = 0; j < sizeof(long); j++, errno = 0) {
					...
					if ((ptraced_long = ptrace(PTRACE_PEEKDATA, pid,
							ptrace_address - j, NULL)) == -1L
	...
	if (reqaddr + sizeof(int64_t) <= last_address_gathered) {
		...
		for (i = 0; i < sizeof(int64_t); ++i) {
			uint8_t val =
					(i < successful_gathering) ?
							peekbuf.cache[reqaddr - peekbuf.base + i] : 0;
...
/* This is the function that handles when you enter a value (or >, <, =) for the second or later time (i.e. when there's already a list of matches); it reduces the list to those that still match. It returns false on failure to attach, detach, or reallocate memory, otherwise true.
 "value" is what to compare to. It is meaningless when the match type is not MATCHEXACT. */bool checkmatches(
		globals_t * vars, scan_match_type_t match_type,
	...
	while (tmp_swath_index->number_of_bytes) {
		...
		tmp_swath_index =
				(matches_and_old_values_swath *) (&tmp_swath_index->data[tmp_swath_index->number_of_bytes]);
	}
	...
	while (reading_swath.first_byte_in_child) {
		...
		if (EXPECT(peekdata(vars->target, address, &data_value) == false, false)) {
			...
			//show_debug("read data from address: %p  value : %x \n",address,data_value);
			value_t old_val = data_to_val_aux(reading_swath_index,
					reading_iterator,
					reading_swath.number_of_bytes /* ,MATCHES_AND_VALUES */);
			...
			match_length = (*g_scan_routine)(&data_value, &old_val, uservalue,
					&checkflags, address);
		}
		...
		if (match_length > 0) {
			...
			old_value_and_match_info new_value = { get_u8b(&data_value),
					checkflags };
			writing_swath_index = add_element((&vars->matches),
					writing_swath_index, address,
					&new_value /* ,MATCHES_AND_VALUES */);
		...
		if (reading_iterator >= reading_swath.number_of_bytes) {
			assert(
					((matches_and_old_values_swath *)(&reading_swath_index->data[reading_swath.number_of_bytes]))->number_of_bytes >= 0);
			reading_swath_index =
					(matches_and_old_values_swath *) (&reading_swath_index->data[reading_swath.number_of_bytes]);
	...
	if (!(vars->matches = null_terminate(vars->matches,
			writing_swath_index /* ,MATCHES_AND_VALUES */))) {
		show_error(
				"memory allocation error while reducing matches-array size\n");
		return false;
	}
...
/* searchregions() performs an initial search of the process for values matching value */bool searchregions(
		globals_t * vars, scan_match_type_t match_type,
		const uservalue_t *uservalue) {
	matches_and_old_values_swath *writing_swath_index;
	int required_extra_bytes_to_record = 0;
	...
	while (n) {
		total_size += ((region_t *) (n->data))->size
	...
	while (n) {
		...
		show_info("size is :%d start is :%p \n", r->size, r->start);
		while (nread < r->size) {
			if ((len = readregion(vars->target, data + nread, r->size - nread,
					(unsigned long) (r->start + nread))) == -1) {
		...
		show_info("%02u/%02u searching %#10lx - %#10lx.", ++regnum,
				vars->regions->size, (unsigned long) r->start,
				(unsigned long) r->start + r->size);
		fflush(stderr);
		//show_info("nread is :%d \n",nread);
		...
		for (offset = 0; offset < nread; offset++) {
			...
			// show_info("address is :%p \n",address);
#if HAVE_PROCMEM           
			data_value.int64_value = *((int64_t *) (&data[offset]));

			/* Mark which values this can't be */
			if (EXPECT((nread - offset < sizeof(int64_t)), false)) {
				data_value.flags.u64b = data_value.flags.s64b =
						data_value.flags.f64b = 0;

				if (nread - offset < sizeof(int32_t)) {
					data_value.flags.u32b = data_value.flags.s32b =
							data_value.flags.f32b = 0;
			...
			if (EXPECT(((match_length = (*g_scan_routine)(&data_value, NULL, uservalue, &checkflags, address)) > 0), false)) {
				...
				old_value_and_match_info new_value = { get_u8b(&data_value),
						checkflags };
				writing_swath_index = add_element((&vars->matches),
						writing_swath_index, r->start + offset, &new_value);
...
bool setaddr(pid_t target, void *addr, const value_t * to) {
	...
	if (saved.flags.u64b && to->flags.u64b) {
		set_u64b(&saved, get_u64b(to));
	} else if (saved.flags.s64b && to->flags.s64b) {
		set_s64b(&saved, get_s64b(to));
	} else if (saved.flags.f64b && to->flags.f64b) {
		set_s64b(&saved, get_s64b(to));
	} /* *((int64_t *)&(to->float64_value))); } */
	else if (saved.flags.u32b && to->flags.u32b) {
		set_u32b(&saved, get_u32b(to));
	} else if (saved.flags.s32b && to->flags.s32b) {
		set_s32b(&saved, get_s32b(to));
	} else if (saved.flags.f32b && to->flags.f32b) {
		set_s32b(&saved, get_s32b(to));
	} /* *((int32_t *)&(to->float32_value))); } */
	else if (saved.flags.u16b && to->flags.u16b) {
		set_u16b(&saved, get_u16b(to));
	} else if (saved.flags.s16b && to->flags.s16b) {
		set_s16b(&saved, get_s16b(to));
	} else if (saved.flags.u8b && to->flags.u8b) {
		set_u8b(&saved, get_u8b(to));
	} else if (saved.flags.s8b && to->flags.s8b) {
		set_s8b(&saved, get_s8b(to));
	} else {
	...
	for (i = 0; i < sizeof(saved.int64_value); i += sizeof(long)) {
		if (ptrace(PTRACE_POKEDATA, target, addr + i,
				*(long *) (((int8_t *) &saved.int64_value) + i)) == -1L) {
			return false;
		}
	}
...
/* TODO: may use /proc/<pid>/mem here */bool write_array(pid_t target,
		void *addr, const void *data, int len) {
	int i, j;
	long peek_value;
	...
	for (i = 0; i + sizeof(long) < len; i += sizeof(long)) {
		if (ptrace(PTRACE_POKEDATA, target, addr + i, *(long *) (data + i))
				== -1L) {
			return false;
		}
	}
	...
	if (len - i > 0) /* something left (shorter than a long) */
	{
		...
		} else /* we have to play with bits... */
		{
			...
			for (j = 0; j <= sizeof(long) - (len - i); ++j) {
				errno = 0;
				if (((peek_value = ptrace(PTRACE_PEEKDATA, target, addr - j,
						NULL)) == -1L) && (errno != 0)) {
				...
				} else /* peek success */
				{
					...
					if (ptrace(PTRACE_POKEDATA, target, addr - j, peek_value)
							== -1L) {
						show_error("write_array failed.\n");
						return false;
					}
